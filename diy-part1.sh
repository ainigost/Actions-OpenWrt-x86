...
wget -qO- -t1 -T2 "https://api.github.com/repos/ainigost/Actions-OpenWrt-x86/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g;s/v//g'  > /tmp/cloud_ts_version
if [ -s  "/tmp/cloud_ts_version" ]; then
	...
	DEV_URL=https://github.com/ainigost/Actions-OpenWrt-x86/releases/download/${new_version}/openwrt_x86-64-${new_version}_sta_Lenyu.img.gz
	DEV_UEFI_URL=https://github.com/ainigost/Actions-OpenWrt-x86/releases/download/${new_version}/openwrt_x86-64-${new_version}_uefi-gpt_sta_Lenyu.img.gz
	openwrt_sta=https://github.com/ainigost/Actions-OpenWrt-x86/releases/download/${new_version}/openwrt_sta.md5
	openwrt_sta_uefi=https://github.com/ainigost/Actions-OpenWrt-x86/releases/download/${new_version}/openwrt_sta_uefi.md5
else
	echo "请检测网络或重试！"
	exit 1
fi
